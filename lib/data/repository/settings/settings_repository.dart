...
class SettingsRepository {
  ...
  AsyncResult<void> saveApiConfig(String host, String token) async {
    ...
    if (res.isError()) {
      _log.severe("保存API配置失败(host)", res.exceptionOrNull());
      return res;
    }

    res = await _prefsService.setReadeckApiToken(token);
    if (res.isError()) {
      _log.severe("保存API配置失败(token)", res.exceptionOrNull());
      return res;
    }
  ...
  AsyncResult<(String, String)> getApiConfig() async {
    ...
    if (host.isError()) {
      _log.severe("获取API配置失败(host)", host.exceptionOrNull());
      return Failure(Exception(host.exceptionOrNull()));
    }

    var token = await _prefsService.getReadeckApiToken();
    if (token.isError()) {
      _log.severe("获取API配置失败(token)", token.exceptionOrNull());
