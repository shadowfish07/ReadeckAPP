...
在我提出需求变更后，应当先更新本文档的功能描述，再进行需求开发。

# 功能描述

## 首页

- 首页展示每日阅读的书签列表，每天会随机抽取 5 个未读书签，书签在次日重置。
- 点击书签，可以跳转到对应的网页
- 书签可以标记喜爱，或者取消喜爱。
- 书签可以标记归档，归档后的书签不会展示在首页。
- 在标记喜爱、归档等涉及 API 更新操作时，需要在更新请求完毕后，异步发请求更新本地数据。
- 首页在发送 GET 请求时，在标题区有加载时提示
- 每日阅读内容全部归档后，首页展示刷新一组内容的入口。
...
- 可配置项
  - 主题
  - API 配置
    > 进入独立页面，配置 API 地址和 token。配置完毕后，返回主页，主页数据应触发自动刷新。
- 关于
